    ...
    <div class="pion-demo-controls">
      ...
      <div class="pion-demo-outputs">
        <div class="pion-demo-output muon">
          ...
          <div class="out-algo">
            <div class="out-algo-step">5 &times; Newton&ndash;Schulz</div>
            <code class="out-algo-line">&sigma; &larr; 3.4445&sigma; &minus; 4.7750&sigma;<sup>3</sup></code>
            <code class="out-algo-line out-algo-cont">+ 2.0315&sigma;<sup>5</sup></code>
          </div>
        ...
        <div class="pion-demo-output pion">
          ...
          <div class="out-algo">
            <div class="out-algo-step">1 &times; Promotion</div>
            <code class="out-algo-line">&sigma; &larr; 1.875&sigma; &minus; 1.25&sigma;<sup>3</sup></code>
            <code class="out-algo-line out-algo-cont">+ 0.375&sigma;<sup>5</sup></code>
            <div class="out-algo-step">+ 4 &times; Suppression</div>
            <code class="out-algo-line">&sigma; &larr; 2.5&sigma;<sup>3</sup> &minus; 1.5&sigma;<sup>5</sup></code>
          </div>
          <div class="out-result-wrap">
            <div class="out-divider"></div>
            <div class="out-result">
  ...
  <div class="axis-track">
    ...
    <div class="axis-to">
      <div class="ax-mod-to">
        ...
        <svg class="ax-mod-arm" viewBox="0 0 42 42" aria-hidden="true">
          <rect x="13" y="34" width="16" height="6" rx="1.5" fill="rgba(68,133,199,0.18)" stroke="#4485C7" stroke-width="1.4"/>
          <line x1="21" y1="34" x2="14" y2="22" stroke="#4485C7" stroke-width="2.4" stroke-linecap="round"/>
          <circle cx="21" cy="34" r="2" fill="#4485C7"/>
          <line x1="14" y1="22" x2="28" y2="14" stroke="#4485C7" stroke-width="2.4" stroke-linecap="round"/>
          <circle cx="14" cy="22" r="2" fill="#4485C7"/>
          <line x1="28" y1="14" x2="33" y2="9" stroke="#4485C7" stroke-width="2.2" stroke-linecap="round"/>
          <line x1="28" y1="14" x2="33" y2="19" stroke="#4485C7" stroke-width="2.2" stroke-linecap="round"/>
          <circle cx="28" cy="14" r="1.6" fill="#4485C7"/>
        </svg>
      </div>
...
VLA training combines axes 1 and 2 (adds the vision and action modality and replaces classification with regression loss), and RLVR isolates axis 3 (keeps the LLM and its tokenizer but switches from supervised learning to reinforcement learning). The rest of the page asks the same question along each axis: **does Muon's uniform spectral whitening still help, or does it become the wrong inductive bias?**
...
**Pseudocode.** For reference, we write out the full procedures below. Algorithm 1 is Muon's standard NS iteration; Pion only replaces the inner NS loop with a two stage high pass version. Algorithm 2 is the default mode used for VLA training, and Algorithm 3 is the per head mode used for RLVR post training. The total iteration count is fixed to \(k = 5\), split into \(k_{\mathrm{p}}\) Promotion steps and \(k_{\mathrm{s}} = k - k_{\mathrm{p}}\) Suppression steps.
...
<div class="algo-block">
  ...
  <ol class="algo-steps">
    ...
    <li class="ind-1"><span class="line">\(\mathbf{G}_t \leftarrow \nabla_{\boldsymbol{\Theta}} \mathcal{L}_t(\boldsymbol{\Theta}_{t-1});\quad \mathbf{M}_t \leftarrow \mu\, \mathbf{M}_{t-1} + \mathbf{G}_t\)</span></li>
    <li class="ind-1"><span class="line">\(\mathbf{X} \leftarrow \mathbf{M}_t / (\lVert \mathbf{M}_t \rVert_F + \epsilon)\)</span><span class="comment">spectral pre-normalization</span></li>
...
<div class="algo-block">
  <div class="algo-header">
    ...
    <span class="algo-title">Pion Optimizer (default mode: high pass NS on the whole matrix)</span>
  </div>
  <div class="algo-require">
    <span class="kw">Require:</span>&nbsp; learning rate \(\eta\), momentum coefficient \(\mu\), promotion steps \(k_{\mathrm{p}}\)
  </div>
  <ol class="algo-steps">
    <li><span class="line">\(k_{\mathrm{s}} \leftarrow 5 - k_{\mathrm{p}};\quad \mathbf{M}_0 \leftarrow \mathbf{0}\)</span><span class="comment">total iterations fixed to \(k = 5\)</span></li>
    <li><span class="line"><span class="kw">for</span> \(t = 1, 2, \dots\) <span class="kw">do</span></span></li>
    <li class="ind-1"><span class="line">\(\mathbf{G}_t \leftarrow \nabla_{\boldsymbol{\Theta}} \mathcal{L}_t(\boldsymbol{\Theta}_{t-1});\quad \mathbf{M}_t \leftarrow \mu\, \mathbf{M}_{t-1} + \mathbf{G}_t\)</span></li>
    <li class="ind-1"><span class="line">\(\mathbf{X} \leftarrow \mathbf{M}_t / (\lVert \mathbf{M}_t \rVert_F + \epsilon)\)</span><span class="comment">spectral pre-normalization</span></li>
    <li class="ind-1"><span class="line"><span class="kw">for</span> \(i = 1, \dots, k_{\mathrm{p}}\) <span class="kw">do</span></span><span class="comment">stage 1: Promotion, \((a_{\mathrm{p}}, b_{\mathrm{p}}, c_{\mathrm{p}}) = (1.875,\ -1.25,\ 0.375)\)</span></li>
    <li class="ind-2"><span class="line">\(\mathbf{X} \leftarrow a_{\mathrm{p}}\, \mathbf{X} + b_{\mathrm{p}}\, \mathbf{X}\mathbf{X}^\top \mathbf{X} + c_{\mathrm{p}}\, \mathbf{X}(\mathbf{X}^\top \mathbf{X})^2\)</span></li>
    <li class="ind-1"><span class="line"><span class="kw">end for</span></span></li>
    <li class="ind-1"><span class="line"><span class="kw">for</span> \(j = 1, \dots, k_{\mathrm{s}}\) <span class="kw">do</span></span><span class="comment">stage 2: Suppression, \((a_{\mathrm{s}}, b_{\mathrm{s}}, c_{\mathrm{s}}) = (0,\ 2.5,\ -1.5)\)</span></li>
    <li class="ind-2"><span class="line">\(\mathbf{X} \leftarrow a_{\mathrm{s}}\, \mathbf{X} + b_{\mathrm{s}}\, \mathbf{X}\mathbf{X}^\top \mathbf{X} + c_{\mathrm{s}}\, \mathbf{X}(\mathbf{X}^\top \mathbf{X})^2\)</span></li>
    <li class="ind-1"><span class="line"><span class="kw">end for</span></span></li>
    <li class="ind-1"><span class="line">\(\boldsymbol{\Theta}_t \leftarrow \boldsymbol{\Theta}_{t-1} - \eta\, \mathbf{X}\)</span></li>
    <li><span class="line"><span class="kw">end for</span></span></li>
    <li><span class="line"><span class="kw">return</span> \(\boldsymbol{\Theta}_t\)</span></li>
...
<div class="algo-block">
  <div class="algo-header">
    ...
    <span class="algo-title">Pion Optimizer (per head mode: per head high pass NS on attention projections)</span>
  </div>
  <div class="algo-require">
    <span class="kw">Require:</span>&nbsp; learning rate \(\eta\), momentum coefficient \(\mu\), promotion steps \(k_{\mathrm{p}}\), number of heads \(H\)
  </div>
  <ol class="algo-steps">
    <li><span class="line">\(k_{\mathrm{s}} \leftarrow 5 - k_{\mathrm{p}};\quad \mathbf{M}_0 \leftarrow \mathbf{0}\)</span><span class="comment">total iterations fixed to \(k = 5\)</span></li>
    <li><span class="line"><span class="kw">for</span> \(t = 1, 2, \dots\) <span class="kw">do</span></span></li>
    <li class="ind-1"><span class="line">\(\mathbf{G}_t \leftarrow \nabla_{\boldsymbol{\Theta}} \mathcal{L}_t(\boldsymbol{\Theta}_{t-1});\quad \mathbf{M}_t \leftarrow \mu\, \mathbf{M}_{t-1} + \mathbf{G}_t\)</span></li>
    <li class="ind-1"><span class="line">\(\{\mathbf{M}_t^h\}_{h=1}^{H} \leftarrow \mathrm{Reshape}(\mathbf{M}_t)\)</span><span class="comment">split attention projection along the head dim</span></li>
    <li class="ind-1"><span class="line">\(\mathbf{X}^h \leftarrow \mathbf{M}_t^h / (\lVert \mathbf{M}_t^h \rVert_F + \epsilon),\ \forall\, h \in \{1, \dots, H\}\)</span><span class="comment">per head pre-normalization</span></li>
    <li class="ind-1"><span class="line"><span class="kw">for</span> \(i = 1, \dots, k_{\mathrm{p}}\) <span class="kw">do</span></span><span class="comment">stage 1: Promotion, batched over \(H\)</span></li>
    <li class="ind-2"><span class="line">\(\mathbf{X}^h \leftarrow a_{\mathrm{p}}\, \mathbf{X}^h + b_{\mathrm{p}}\, \mathbf{X}^h(\mathbf{X}^h)^\top \mathbf{X}^h + c_{\mathrm{p}}\, \mathbf{X}^h\bigl((\mathbf{X}^h)^\top \mathbf{X}^h\bigr)^2,\ \forall\, h\)</span></li>
    <li class="ind-1"><span class="line"><span class="kw">end for</span></span></li>
    <li class="ind-1"><span class="line"><span class="kw">for</span> \(j = 1, \dots, k_{\mathrm{s}}\) <span class="kw">do</span></span><span class="comment">stage 2: Suppression, batched over \(H\)</span></li>
    <li class="ind-2"><span class="line">\(\mathbf{X}^h \leftarrow a_{\mathrm{s}}\, \mathbf{X}^h + b_{\mathrm{s}}\, \mathbf{X}^h(\mathbf{X}^h)^\top \mathbf{X}^h + c_{\mathrm{s}}\, \mathbf{X}^h\bigl((\mathbf{X}^h)^\top \mathbf{X}^h\bigr)^2,\ \forall\, h\)</span></li>
    <li class="ind-1"><span class="line"><span class="kw">end for</span></span></li>
    <li class="ind-1"><span class="line">\(\mathbf{X} \leftarrow \mathrm{Reshape}^{-1}\bigl(\{\mathbf{X}^h\}_{h=1}^{H}\bigr) \in \mathbb{R}^{m \times n}\)</span></li>
    <li class="ind-1"><span class="line">\(\boldsymbol{\Theta}_t \leftarrow \boldsymbol{\Theta}_{t-1} - \eta\, \mathbf{X}\)</span></li>
    <li><span class="line"><span class="kw">end for</span></span></li>
    <li><span class="line"><span class="kw">return</span> \(\boldsymbol{\Theta}_t\)</span></li>
  </ol>
...
* **Muon** 已经是 LLM 预训练里事实上的"矩阵感知"优化器。LLM 预训练说到底就是文本上的下一 token 分类，一种监督学习任务。Muon 用几步 Newton–Schulz (NS) 迭代正交化动量矩阵，把动量的每一个奇异值都推到 1。
...
**统一的谱诊断。** 局限 1 和局限 2 的来源不同：一个出在模态/损失轴上的低有效秩，另一个出在范式轴上的低 SNR；但它们在频谱上长得一模一样。在 $$\mathbf{M}_t$$ 的 SVD 里，**前面少数几个**奇异值承载着信息量大的下降方向，**后面长长一串**小奇异值则被噪声主导（erank 低时它表现为谱底，SNR 低时它表现为随机估计噪声）。Muon 的 $$\mathrm{msign}$$ 把尾部抬到和头部一样高，在这两种情形下都把更新搅浑了。自然的对策就是**谱高通**：把头部锁在 1 附近，把尾部一路压到 0。
...
这两条结论刚好互补：**谱高通**是 RLVR 稳定下来的主驱动（per-head Muon 仍然崩），而 **per-head reshape** 是辅助机制：它把预训练遗留的 head 异质性保住，把 per-head Pion 推到了 AdamW 之上。

**伪代码。** 为方便引用，下面把完整流程写出来。算法 1 是 Muon 标准的 NS 迭代；Pion 只是把内层 NS 换成两段式高通版本。算法 2 是默认模式，用于 VLA 训练；算法 3 是 per-head 模式，用于 RLVR 后训练。总迭代步数严格固定为 \(k = 5\)，拆成 \(k_{\mathrm{p}}\) 步 Promotion 和 \(k_{\mathrm{s}} = k - k_{\mathrm{p}}\) 步 Suppression。
...
<div class="algo-block">
  <div class="algo-header">
    ...
    <span class="algo-title">Pion 优化器（默认模式：对整块矩阵跑高通 NS）</span>
  </div>
  <div class="algo-require">
    <span class="kw">Require:</span>&nbsp; 学习率 \(\eta\)，动量系数 \(\mu\)，Promotion 步数 \(k_{\mathrm{p}}\)
  </div>
  <ol class="algo-steps">
    <li><span class="line">\(k_{\mathrm{s}} \leftarrow 5 - k_{\mathrm{p}};\quad \mathbf{M}_0 \leftarrow \mathbf{0}\)</span><span class="comment">总迭代步数严格固定为 \(k = 5\)</span></li>
    <li><span class="line"><span class="kw">for</span> \(t = 1, 2, \dots\) <span class="kw">do</span></span></li>
    <li class="ind-1"><span class="line">\(\mathbf{G}_t \leftarrow \nabla_{\boldsymbol{\Theta}} \mathcal{L}_t(\boldsymbol{\Theta}_{t-1});\quad \mathbf{M}_t \leftarrow \mu\, \mathbf{M}_{t-1} + \mathbf{G}_t\)</span></li>
    <li class="ind-1"><span class="line">\(\mathbf{X} \leftarrow \mathbf{M}_t / (\lVert \mathbf{M}_t \rVert_F + \epsilon)\)</span><span class="comment">谱预归一化</span></li>
    <li class="ind-1"><span class="line"><span class="kw">for</span> \(i = 1, \dots, k_{\mathrm{p}}\) <span class="kw">do</span></span><span class="comment">第一阶段 Promotion，\((a_{\mathrm{p}}, b_{\mathrm{p}}, c_{\mathrm{p}}) = (1.875,\ -1.25,\ 0.375)\)</span></li>
    <li class="ind-2"><span class="line">\(\mathbf{X} \leftarrow a_{\mathrm{p}}\, \mathbf{X} + b_{\mathrm{p}}\, \mathbf{X}\mathbf{X}^\top \mathbf{X} + c_{\mathrm{p}}\, \mathbf{X}(\mathbf{X}^\top \mathbf{X})^2\)</span></li>
    <li class="ind-1"><span class="line"><span class="kw">end for</span></span></li>
    <li class="ind-1"><span class="line"><span class="kw">for</span> \(j = 1, \dots, k_{\mathrm{s}}\) <span class="kw">do</span></span><span class="comment">第二阶段 Suppression，\((a_{\mathrm{s}}, b_{\mathrm{s}}, c_{\mathrm{s}}) = (0,\ 2.5,\ -1.5)\)</span></li>
    <li class="ind-2"><span class="line">\(\mathbf{X} \leftarrow a_{\mathrm{s}}\, \mathbf{X} + b_{\mathrm{s}}\, \mathbf{X}\mathbf{X}^\top \mathbf{X} + c_{\mathrm{s}}\, \mathbf{X}(\mathbf{X}^\top \mathbf{X})^2\)</span></li>
    <li class="ind-1"><span class="line"><span class="kw">end for</span></span></li>
    <li class="ind-1"><span class="line">\(\boldsymbol{\Theta}_t \leftarrow \boldsymbol{\Theta}_{t-1} - \eta\, \mathbf{X}\)</span></li>
    <li><span class="line"><span class="kw">end for</span></span></li>
...
<div class="algo-block">
  <div class="algo-header">
    ...
    <span class="algo-title">Pion 优化器（per-head 模式：在 attention 投影上按 head 跑高通 NS）</span>
  </div>
  <div class="algo-require">
  ...
  <ol class="algo-steps">
    ...
    <li><span class="line"><span class="kw">for</span> \(t = 1, 2, \dots\) <span class="kw">do</span></span></li>
    <li class="ind-1"><span class="line">\(\mathbf{G}_t \leftarrow \nabla_{\boldsymbol{\Theta}} \mathcal{L}_t(\boldsymbol{\Theta}_{t-1});\quad \mathbf{M}_t \leftarrow \mu\, \mathbf{M}_{t-1} + \mathbf{G}_t\)</span></li>
    <li class="ind-1"><span class="line">\(\{\mathbf{M}_t^h\}_{h=1}^{H} \leftarrow \mathrm{Reshape}(\mathbf{M}_t)\)</span><span class="comment">沿 head 维度切分 attention 投影</span></li>
    <li class="ind-1"><span class="line">\(\mathbf{X}^h \leftarrow \mathbf{M}_t^h / (\lVert \mathbf{M}_t^h \rVert_F + \epsilon),\ \forall\, h \in \{1, \dots, H\}\)</span><span class="comment">per-head 预归一化</span></li>
    <li class="ind-1"><span class="line"><span class="kw">for</span> \(i = 1, \dots, k_{\mathrm{p}}\) <span class="kw">do</span></span><span class="comment">第一阶段 Promotion，沿 \(H\) 做 batch</span></li>
    <li class="ind-2"><span class="line">\(\mathbf{X}^h \leftarrow a_{\mathrm{p}}\, \mathbf{X}^h + b_{\mathrm{p}}\, \mathbf{X}^h(\mathbf{X}^h)^\top \mathbf{X}^h + c_{\mathrm{p}}\, \mathbf{X}^h\bigl((\mathbf{X}^h)^\top \mathbf{X}^h\bigr)^2,\ \forall\, h\)</span></li>
    <li class="ind-1"><span class="line"><span class="kw">end for</span></span></li>
    <li class="ind-1"><span class="line"><span class="kw">for</span> \(j = 1, \dots, k_{\mathrm{s}}\) <span class="kw">do</span></span><span class="comment">第二阶段 Suppression，沿 \(H\) 做 batch</span></li>
    <li class="ind-2"><span class="line">\(\mathbf{X}^h \leftarrow a_{\mathrm{s}}\, \mathbf{X}^h + b_{\mathrm{s}}\, \mathbf{X}^h(\mathbf{X}^h)^\top \mathbf{X}^h + c_{\mathrm{s}}\, \mathbf{X}^h\bigl((\mathbf{X}^h)^\top \mathbf{X}^h\bigr)^2,\ \forall\, h\)</span></li>
    <li class="ind-1"><span class="line"><span class="kw">end for</span></span></li>
    <li class="ind-1"><span class="line">\(\mathbf{X} \leftarrow \mathrm{Reshape}^{-1}\bigl(\{\mathbf{X}^h\}_{h=1}^{H}\bigr) \in \mathbb{R}^{m \times n}\)</span></li>
    <li class="ind-1"><span class="line">\(\boldsymbol{\Theta}_t \leftarrow \boldsymbol{\Theta}_{t-1} - \eta\, \mathbf{X}\)</span></li>
