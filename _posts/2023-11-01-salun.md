...
$$
    \displaystyle \text{minimize}_\boldsymbol{\theta}_{\mathrm{u}} ~ L_\text{SalUn}^{(1)} (\boldsymbol{\theta}_\mathrm{u}) \mathrel{\mathop:}=  \mathbb E_{(\mathbf x, y) \sim \mathcal{D}_\mathrm{f}, y^\prime \neq y} \left [ \ell_\mathrm{CE}(\boldsymbol{\theta}_\mathrm{u}; \mathbf x, y^\prime) \right ]
$$

$$
    \displaystyle \text{minimize}_\boldsymbol{\theta}_{\mathrm{u}} ~  L_\text{SalUn}^{(2)} (\boldsymbol{\theta}_\mathrm{u}) \mathrel{\mathop:}=  \mathbb{E}_{(\mathbf x, c) \sim \mathcal D_\mathrm{f}, t, \epsilon \sim \mathcal{N}(0,1), c^\prime \neq c  } \left [ \| \epsilon_\boldsymbol{\theta}_{\mathrm{u}}(\mathbf x_t | c^\prime) - \epsilon_\boldsymbol{\theta}_{\mathrm{u}}(\mathbf x_t | c) \|_2^2 \right ] + \alpha \ell_\mathrm{MSE}(\boldsymbol{\theta}_\mathrm{u}; \mathcal D_\mathrm{r})
$$