...
$$
 ...
 where $ \mathbf 1 (\mathbf g \geq \gamma ) $ is an element-wise indicator function which yields a value of $1$ for the $i$-th element if $ g_i \geq \gamma $ and $ 0 $ otherwise, $\lvert·\rvert$ is an element-wise absolute value operation, and $\gamma > 0$ is a hard threshold.
