...
Muon is the de facto **matrix aware** optimizer for **LLM pretraining**, which is essentially next token classification on text under supervised learning. The whole construction is built around one observation: when momentum is treated as a matrix rather than a flat vector, the natural notion of steepest descent under the spectral norm orthogonalizes the singular vectors of the momentum and pushes every nonzero singular value to one.
...
LLM pretraining is a **classification** task (predict the next token), uses **text** as the only modality, and runs under **supervised learning**. With clean dense supervision and full rank text gradients, Muon's uniform whitening is a sensible inductive bias. But LLM pretraining is only one slice of deep learning; how Muon behaves under different **modalities**, **losses**, and **learning paradigms** remains an open question worth exploring.
...
<div class="pion-demo" data-muon-labels="被白化为 1|从 0 拉起|越过 1（震荡）" data-pion-labels="保留（高通）|过滤（被压制）|保留（高通）">
  <div class="pion-demo-body">
    <div class="pion-demo-controls">
      <div class="pion-demo-intro">
        挑一个 <span class="kbd">&sigma; &isin; [0, 1]</span>，看同一个 <span class="kbd">&sigma;</span> 经过 Muon 的 NS 迭代（5 步）和 Pion 的高通 NS（<span class="kbd">k<sub>p</sub> = 1, k<sub>s</sub> = 4</span>）分别会落在哪。Muon 想把所有 <span class="kbd">&sigma;</span> 都拽到 1；Pion 留下头部，扔掉尾部。
      </div>
      <div class="pion-demo-slider-row">
        <span class="sigma-label">&sigma;</span>
...
Muon 现在是 LLM 预训练里事实上的"**矩阵感知**"优化器，而 LLM 预训练说到底就是文本上的下一 token 分类，是一种监督学习。它的整个构造都围绕一个观察展开：把动量当作矩阵而不是拉平后的向量，那么"在谱范数下做最速下降"这个最自然的更新方式，等价于把动量正交化，把它的每一个非零奇异值都推到 1。
...
也就是说，设计一套 NS 迭代等价于在 $$[0, 1]$$ 上设计 $$f$$。Muon 的 NS 被特意构造成：迭代几次之后，能把任意 $$\sigma \in (0, 1]$$ 都赶到 1 附近。这条标量曲线长什么样（以及 Pion 拿什么把它换掉），正是后面 [Figure 1](#fig-pion-iter) 想画清楚的事。

### 跳出预训练的三条轴

LLM 预训练有三个默认设定：任务是**分类**（预测下一个 token），模态只有**文本**，范式是**监督学习**。当 token 级监督密集、文本梯度满秩时，把所有奇异值都拍到 1 是个挺合理的归纳偏置。但 LLM 预训练终究只是深度学习的一个切片，Muon 在不同**模态**、不同**损失**、不同**学习范式**下究竟表现如何，仍然是一个值得探索的问题：
...
VLA 同时改变了模态和损失（多了一种视觉与动作模态，又把分类损失换成回归损失）；RLVR 则单独改变了学习范式（仍然用 LLM 和它的 tokenizer，只把监督学习换成强化学习）。沿每条轴我们问的是同一个问题：**Muon 那种"把所有奇异值都拍平"的均匀白化，到底还有用，还是已经成了错误的归纳偏置？** 下面两小节分别给出 VLA 和 RLVR 上的诊断与代表性局限。
...
为了把两种学习范式放在一起比较，我们看每一步梯度在某层权重上的信噪比：
...
<div class="callout" markdown="1">
**局限 2（学习范式）。** Muon 的均匀白化会放大低 SNR 的 RLVR 梯度里的噪声方向，注定不适合对噪声敏感的后训练；同时它也丢掉了预训练遗留下来的 per-head 异质性。
</div>

## 方法

局限 1 和局限 2 在频谱上指向同一种病：尾部噪声被均匀白化抬起来。这促使我们对内层 NS 迭代做一次统一的重设计。下面四小节依次：(i) 给出统一的谱诊断；(ii) 设计两段式高通多项式；(iii) 推广到 attention 投影上的 per-head 模式；(iv) 写出完整的伪代码。

### 统一的谱诊断

局限 1 和局限 2 的来源不同：一个出在模态/损失轴上的低有效秩，另一个出在学习范式轴上的低 SNR；但它们在频谱上长得一模一样。在 $$\mathbf{M}_t$$ 的 SVD 里，**前面少数几个**奇异值承载着信息量大的下降方向，**后面长长一串**小奇异值则被噪声主导（erank 低时它表现为谱底，SNR 低时它表现为随机估计噪声）。Muon 的 $$\mathrm{msign}$$ 把尾部抬到和头部一样高，在这两种情形下都把更新搅浑了。自然的对策就是**谱高通**：把头部锁在 1 附近，把尾部一路压到 0。

### 两段式高通 NS

既然 NS 每一步都是用 $$f(\sigma; a, b, c) = a\sigma + b\sigma^3 + c\sigma^5$$ 去重塑 $$\sigma$$，那 NS 的设计就归结到对 $$f$$ 的设计。靠单一的这种五次多项式，做不出我们想要的那种锐利高通；所以 Pion 把默认的 $$k = 5$$ 步 NS 拆成两段，用不同的系数：
...
<p class="fig-caption">视频 1. 同一个 LIBERO episode（ep1373）在三种优化器下的执行轨迹。这正是表 1 中 LIBERO Plus 的一个具体实例：只有 Pion 稳定完成任务，AdamW 与 Muon 都在抓取或放置阶段失败。</p>
