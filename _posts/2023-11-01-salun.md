...
$$
  \hat{\epsilon}_{\btheta}(\mathbf x_t | c) = (1-w) {\epsilon}_{\btheta}(\mathbf x_t | \emptyset ) + w {\epsilon}_{\btheta}(\mathbf x_t | c )
$$

$$
    \mathbf m_{\mathrm{S}}   = \mathbf 1 \left ( \left |  \nabla_{\btheta} \ell_{\mathrm{f}} (\btheta; \mathcal{D}_\mathrm{f}) \left . \right |_{\btheta = \thetafull } \right  | \geq  \gamma \right )
$$

$$
    \thetaunl = \underbrace{\mathbf m_{\mathrm{S}} \odot \btheta}_\text{salient weights} + \underbrace{ (\mathbf 1 - \mathbf m_{\mathrm{S}}) \odot \thetafull }_\text{original weights}
$$

$$
     \text{Classification: } \ell_\mathrm{f} (\btheta ; \mathcal D_\mathrm{f}) = \mathbb E_{(\mathbf x, y) \sim \mathcal D_\mathrm{f}} [ \ell_{\mathrm{CE}}(\btheta; \mathbf x ,y)];
     \text{Generation: } \ell_\mathrm{f} (\btheta ; \mathcal D_\mathrm{f}) =   \ell_\mathrm{MSE}(\btheta; \mathcal D_\mathrm{f})
$$

$$
    \displaystyle \minimize_{\thetaunl} ~ L_\text{SalUn}^{(1)} (\btheta_\mathrm{u}) \Def  \mathbb E_{(\mathbf x, y) \sim \mathcal{D}_\mathrm{f}, y^\prime \neq y} \left [ \ell_\mathrm{CE}(\btheta_\mathrm{u}; \mathbf x, y^\prime) \right ]
$$

$$
    \displaystyle \minimize_{\thetaunl} ~  L_\text{SalUn}^{(2)} (\btheta_\mathrm{u}) \Def  \mathbb{E}_{(\mathbf x, c) \sim \mathcal D_\mathrm{f}, t, \epsilon \sim \mathcal{N}(0,1), c^\prime \neq c  } \left [ \| \epsilon_{\thetaunl}(\mathbf x_t | c^\prime) - \epsilon_{\thetaunl}(\mathbf x_t | c) \|_2^2 \right ] + \alpha \ell_\mathrm{MSE}(\thetaunl; \mathcal D_\mathrm{r})
$$