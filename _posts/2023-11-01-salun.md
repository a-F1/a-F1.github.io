...
$$
    \mathbf m_{\mathrm{S}}   = \mathbf 1 \left ( \left |  \nabla_{\boldsymbol{\theta}} \ell_{\mathrm{f}} (\boldsymbol{\theta}; \mathcal{D}_\mathrm{f}) \left . \right |_{\boldsymbol{\theta} = \btheta_{\mathrm{o}} } \right  | \geq  \gamma \right )
$$

$$
    \btheta_\mathrm{u} = \underbrace{\mathbf m_{\mathrm{S}} \odot \boldsymbol{\theta}}_\text{salient weights} + \underbrace{ (\mathbf 1 - \mathbf m_{\mathrm{S}}) \odot \thetafull }_\text{original weights}
$$
...
$$
    \displaystyle \text{minimize}_\btheta_{\mathrm{u}} ~ L_\text{SalUn}^{(1)} (\boldsymbol{\theta}_\mathrm{u}) \mathrel{\mathop:}=  \mathbb E_{(\mathbf x, y) \sim \mathcal{D}_\mathrm{f}, y^\prime \neq y} \left [ \ell_\mathrm{CE}(\boldsymbol{\theta}_\mathrm{u}; \mathbf x, y^\prime) \right ]
$$

$$
    \displaystyle \text{minimize}_\btheta_{\mathrm{u}} ~  L_\text{SalUn}^{(2)} (\boldsymbol{\theta}_\mathrm{u}) \mathrel{\mathop:}=  \mathbb{E}_{(\mathbf x, c) \sim \mathcal D_\mathrm{f}, t, \epsilon \sim \mathcal{N}(0,1), c^\prime \neq c  } \left [ \| \epsilon_\btheta_{\mathrm{u}}(\mathbf x_t | c^\prime) - \epsilon_\btheta_{\mathrm{u}}(\mathbf x_t | c) \|_2^2 \right ] + \alpha \ell_\mathrm{MSE}(\btheta_\mathrm{u}; \mathcal D_\mathrm{r})
$$