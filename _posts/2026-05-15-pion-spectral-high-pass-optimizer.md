...
subtitle: "A spectral high pass replacement for Muon's Newton Schulz iteration, designed for the regimes where uniform spectral whitening fails."
date: 2026-05-15
...
affiliations:
  ...
  - id: 4
    name: "IBM Research"
venue: "Preprint, 2026"
notes: "Pion is a drop in replacement for Muon's NS iteration, at the same per step cost."
links:
  - label: "arXiv"
    url: "#"
    icon: "📄"
  ...
  - label: "BibTeX"
    url: "#bibtex"
    icon: "❝"
teaser: "/images/blog/pion/iter_pion_mix.png"
teaser_caption: "Pion replaces Muon's uniform spectral whitening (every singular value pushed to one) with a sharp spectral high pass: leading singular values are anchored at one while the noisy tail is contracted toward zero."
tags:
  - optimizer
  - muon
  - vla
...
<figure>
  ...
  <figcaption>Figure 2. Per module gradient erank along the training trajectory of VLA Adapter on LIBERO Object. Vision (top) is highest, language (middle) is intermediate, action (bottom) is consistently the lowest.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>Figure 3. Test success rate on LIBERO Object at 4.5k training steps with vision and language fixed at AdamW. Muon underperforms AdamW on the action head; Low Rank Muon helps but pays roughly 15x training cost.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>Figure 4. Gradient SNR of SFT versus GRPO under AdamW on Qwen3 1.7B (MATH levels 3 to 5). GRPO has substantially lower SNR than SFT throughout training.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>Figure 5. MATH500 accuracy of GRPO on Qwen3 1.7B with AdamW versus Muon. AdamW improves steadily; Muon collapses to near zero accuracy within a few steps.</figcaption>
</figure>
...
3. **(P3) Boundary concavity.** $$f_{\mathrm{p}}''(1) \leq 0$$ together with (P2) ensures $$\sigma = 1$$ is a maximum so the iteration does not curve upward past one near the boundary.
...
<figure>
  ...
  <figcaption>Figure 6. MATH500 accuracy of AdamW, Muon (default vs per head), and Pion (default vs per head) on Qwen3 1.7B with GRPO on MATH levels 3 to 5. Per head Pion is the only configuration that beats AdamW. Per head mode does not save Muon, since the lack of noise adaptiveness remains the primary issue.</figcaption>
</figure>
...
<div class="fig-grid" style="grid-template-columns: repeat(2, 1fr);">
