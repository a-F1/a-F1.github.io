...
$$
    \thetaunl = \underbrace{\mathbf m_{\mathrm{S}} \odot \boldsymbol{\theta}}_\text{salient weights} + \underbrace{ (\mathbf 1 - \mathbf m_{\mathrm{S}}) \odot \thetafull }_\text{original weights}
$$
...
$$
    \displaystyle \minimize_{\thetaunl} ~ L_\text{SalUn}^{(1)} (\boldsymbol{\theta}_\mathrm{u}) \Def  \mathbb E_{(\mathbf x, y) \sim \mathcal{D}_\mathrm{f}, y^\prime \neq y} \left [ \ell_\mathrm{CE}(\boldsymbol{\theta}_\mathrm{u}; \mathbf x, y^\prime) \right ]
$$

$$
    \displaystyle \minimize_{\thetaunl} ~  L_\text{SalUn}^{(2)} (\boldsymbol{\theta}_\mathrm{u}) \Def  \mathbb{E}_{(\mathbf x, c) \sim \mathcal D_\mathrm{f}, t, \epsilon \sim \mathcal{N}(0,1), c^\prime \neq c  } \left [ \| \epsilon_{\thetaunl}(\mathbf x_t | c^\prime) - \epsilon_{\thetaunl}(\mathbf x_t | c) \|_2^2 \right ] + \alpha \ell_\mathrm{MSE}(\thetaunl; \mathcal D_\mathrm{r})
$$