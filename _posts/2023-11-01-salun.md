...
$$
 ...
 where $$ \mathbf 1 (\mathbf g \geq \gamma ) $$ is an element-wise indicator function 
 <!-- which yields a value of $$1$$ for the $$i$$-th element if $$ g_i \geq \gamma $$ and $$ 0 $$ otherwise, $$|\cdot| $$ is an element-wise absolute value operation, and $$\gamma > 0$$ is a hard threshold. In practice, we have observed that setting $$\gamma$$ to the median of the gradient vector  $$\nabla_{\boldsymbol{\theta}} \ell_{\mathrm{f}} (\boldsymbol{\theta}; \mathcal{D}_\mathrm{f}) \left . \right |_{\boldsymbol{\theta} = \boldsymbol{\theta}_{\mathrm{o}} }$$ is a sufficiently effective choice.  We express the unlearning model $$\boldsymbol{\theta}_\mathrm{u}$$ as -->
